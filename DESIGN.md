...
12. **Skeleton loading** — saat fetch data, semua halaman menampilkan placeholder shimmer (`components/Skeleton.jsx`: `SkeletonList` untuk daftar kartu, `SkeletonChart` untuk grafik, `SkeletonTable` untuk tabel/gantt, `SkeletonForm` untuk form detail task) alih-alih spinner; animasi shimmer via `.skeleton` di `globals.css`, dimatikan pada `prefers-reduced-motion`, tiap grup diberi `role="status"` + `aria-label="Memuat…"`. Spinner `Loading` tetap dipakai untuk cek sesi auth level halaman.
