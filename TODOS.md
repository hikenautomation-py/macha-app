...
- [x] Tes ulang 2026-08-30 #2: DKIM ✅ & MX ✅ `notif.machapp.web.id` (`inbound-amazonses`) & DMARC root `p=none` ✅ sudah terpublikasi (verifikasi dns.google + cloudflare). **SPF BELUM terbit**: TXT `v=spf1 …` di host `notif` tidak ada (query NXDOMAIN) → record SPF yang di-ADD di Domainesia tidak tersimpan di host benar → cek email `SPF=none` → email test masuk **Junk** (Dedy). Koreksi: tambah/normal-kan metode with Host=`notif` & value persis dari dashboard Resend → Restart verification → test ulang. Email masuk Junk = tanda DKIM/DMARC sudah lolos, tinggal SPF. Catatan: API key Resend **send-only (restricted)** → delivery/bounce tak bisa dicek API.
