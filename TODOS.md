...
- [ ] **Keamanan (rotasi rahasia, alasan: key bocor)**: API key Resend `re_T3F6…` sempat bocor plaintext & tercatat di `.commandcode/settings.json` + riwayat git commit `a31323c` (ancestor `origin/main`). Vercel token `vcp_…` juga bocor di file yang sama. **Skenario rotasi lengkap sudah disiapkan di `apicredential.md` §7** — urutan: buat key Resend baru (send-only) → update `.env.local` + Vercel env `EMAIL_API_KEY` + deploy → verifikasi `node --env-file=.env.local _send_test3.mjs` → revoke key lama → (opsional) purge history + rotasi Vercel token.
