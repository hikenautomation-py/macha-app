 ...
 * @param {string} [p.replyTo] alamat email user yang men-trigger notifikasi
 *                             (dipakai sebagai Reply-To; laporan/request tidak memakai ini)
 */
export async function sendEmail({ to, subject, html, text, replyTo }) {
  ...
  try {
    const res = await fetch(RESEND_API_URL, {
      ...
      // Timeout wajar — bila Resend lambat/hang, request user tidak ikut
      // menggantung (fetch dibatalkan, fungsi return null; notifikasi email
...
/** 📋 Task baru → ke pelaksana (assigned_to). Reply-to: atasan (assigned_by). */
export async function emailTaskAssigned(admin, userId, task) {
  const to = await getUserEmail(admin, userId);
  if (!to) return null;
  const replyTo = await getUserEmail(admin, task?.assigned_by);
  const body =
...
/** ✅ Completion report masuk → ke atasan (assigned_by). Reply-to: pelaksana. */
export async function emailReportSubmitted(admin, userId, task, pelaksanaNama) {
  const to = await getUserEmail(admin, userId);
  if (!to) return null;
  const replyTo = await getUserEmail(admin, task?.assigned_to);
  const body =
...
/** 🚨 Problem report → ke atasan (assigned_by), prioritas tinggi. Reply-to: pelaksana. */
...
/** 🎉 Hasil approval (disetujui) → ke pelaksana (assigned_to). Reply-to: atasan (assigned_by). */
...
/** ↩️ Hasil approval (perlu revisi) → ke pelaksana (assigned_to). Reply-to: atasan (assigned_by). */
...
/** 🚨 Laporan umum / request dari seksi lain → ke semua atasan (golongan >= 5). */
export async function emailExternalReport(admin, { type, nama, npk, deskripsi }) {
  const label = type === 'problem' ? 'Laporan masalah umum' : 'Permintaan improvement';
  const { data: atasans } = await admin.from('users').select('id').gte('golongan', 5);
  for (const a of atasans || []) {
    const to = await getUserEmail(admin, a.id);
    if (!to) continue;
    const body =
      row('Jenis', label) +
      row('Nama', esc(nama)) +
    ...
    await sendEmail({
      to,
      subject: `${type === 'problem' ? '🚨' : '💡'} ${label}: dari ${nama}`,
