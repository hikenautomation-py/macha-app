...
// Timeout wajar untuk semua call API Telegram — bila Telegram lambat/hang,
// request user tidak ikut menggantung (fetch dibatalkan, fungsi return null).
...
/**
 * Set menu perintah bot (muncul saat user mengetik "/").
 * Persisten di sisi Telegram sampai di-set ulang; dipanggil ulang tiap ada
 * perintah agar menu selalu sinkron setelah deploy.
 */
export async function setBotCommands() {
  ...
  try {
    const res = await fetch(`${API_BASE}/setMyCommands`, {
      ...
      body: JSON.stringify({
        commands: [
          { command: 'start', description: 'Mulai / daftarkan diri (ketik NPK untuk menautkan akun web)' },
          { command: 'laporan', description: 'Laporkan masalah umum (tanpa akun, cukup nama & NPK)' },
          { command: 'request', description: 'Ajukan permintaan improvement (tanpa akun)' },
          { command: 'help', description: 'Bantuan daftar perintah bot' },
        ],
      }),
      signal: AbortSignal.timeout(FETCH_TIMEOUT_MS),
    });
