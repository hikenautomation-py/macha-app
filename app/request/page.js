...
export default function RequestPage() {
  ...
  async function submit(e) {
    ...
    const res = await apiFetch(null, '/api/external', {
      method: 'POST',
      body: { type: 'improvement', nama: form.nama.trim(), npk: form.npk.trim(), deskripsi: form.deskripsi.trim() },
    });
    setBusy(false);
    if (!res.ok) {
        ...
        <div className="phone-inner">
          ...
          {done ? (
            ...
            <form className="card" onSubmit={submit}>
              ...
              <label className="f-label" style={{ marginTop: 12 }}>Deskripsi improvement</label>
              <textarea
                className="f-input"
                style={{ minHeight: 110 }}
                value={form.deskripsi}
                onChange={set('deskripsi')}
                placeholder="Jelaskan usulan improvement kamu"
              />
